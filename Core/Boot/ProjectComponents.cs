        ...
        public void AddOnAddListener<TModel, TComponent>(ActionListener<TModel, TComponent> listener) where TModel : Model where TComponent : ModelComponent {
            ...
            if (_listeners.TryGetValue(key, out IComponentListeners value)) {
                if (value is ComponentListeners<TModel, TComponent> target) {
                    ...
                    DebugUtility.LogError($"ProjectComponents.AddOnAddListener<{typeof(TModel).Name}, {typeof(TComponent).Name}> - Invalid listeners!");
                }
            } else {
        ...
        public void RemoveOnAddListener<TModel, TComponent>(ActionListener<TModel, TComponent> listener)
            ...
            if (_listeners.TryGetValue(key, out IComponentListeners value)) {
                if (value is ComponentListeners<TModel, TComponent> target) {
                    ...
                    DebugUtility.LogError($"ProjectComponents.RemoveOnAddListener<{typeof(TModel).Name}, {typeof(TComponent).Name}> - Invalid listeners!");
                }
            } else {
                ComponentListeners<TModel, TComponent> target = new ComponentListeners<TModel, TComponent>();
                target.RemoveOnAddListener(listener);
        ...
        public void AddOnRemoveListener<TModel, TComponent>(ActionListener<TModel, TComponent> listener)
            ...
            if (_listeners.TryGetValue(key, out IComponentListeners value)) {
                if (value is ComponentListeners<TModel, TComponent> target) {
                    ...
                    DebugUtility.LogError($"ProjectComponents.AddOnRemoveListener<{typeof(TModel).Name}, {typeof(TComponent).Name}> - Invalid listeners!");
                }
            } else {
                ComponentListeners<TModel, TComponent> target = new ComponentListeners<TModel, TComponent>();
                target.AddOnRemoveListener(listener);
        ...
        public void RemoveOnRemoveListener<TModel, TComponent>(ActionListener<TModel, TComponent> listener)
            ...
            if (_listeners.TryGetValue(key, out IComponentListeners value)) {
                if (value is ComponentListeners<TModel, TComponent> target) {
                    ...
                    DebugUtility.LogError($"ProjectComponents.RemoveOnRemoveListener<{typeof(TModel).Name}, {typeof(TComponent).Name}> - Invalid listeners!");
                }
            } else {
                ComponentListeners<TModel, TComponent> target = new ComponentListeners<TModel, TComponent>();
                target.RemoveOnRemoveListener(listener);
