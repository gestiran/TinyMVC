...
namespace TinyMVC.Loop.Extensions {
    public static class BeginPlayExtension {
        ...
        public static async Task TryBeginPlayAsync<T>(this List<T> collection) {
            for (int i = 0; i < collection.Count; i++) {
                await collection[i].TryBeginPlayAsyncSingle();
            }
        }
        
        public static async Task TryBeginPlayAsync<T>(this T[] collection) {
            for (int i = 0; i < collection.Length; i++) {
                await collection[i].TryBeginPlayAsyncSingle();
            }
        }
        ...
        public static async Task BeginPlayAsync<T>(this List<T> collection) where T : IBeginPlayAsync {
            for (int i = 0; i < collection.Count; i++) {
                try {
        ...
        public static async Task BeginPlayAsync<T>(this T[] collection) where T : IBeginPlayAsync {
            for (int i = 0; i < collection.Length; i++) {
                try {
                    await collection[i].BeginPlay();
                } catch (Exception exception) {
