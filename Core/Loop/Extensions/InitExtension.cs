...
namespace TinyMVC.Loop.Extensions {
    public static class InitExtension {
        ...
        public static async Task TryInitAsync<T>(this List<T> collection) {
            for (int i = 0; i < collection.Count; i++) {
                await collection[i].TryInitAsyncSingle();
            }
        }
        
        public static async Task TryInitAsync<T>(this T[] collection) {
            for (int i = 0; i < collection.Length; i++) {
                await collection[i].TryInitAsyncSingle();
            }
        }
        ...
        public static async Task InitAsync<T>(this List<T> collection) where T : IInitAsync {
            for (int i = 0; i < collection.Count; i++) {
                try {
        ...
        public static async Task InitAsync<T>(this T[] collection) where T : IInitAsync {
            for (int i = 0; i < collection.Length; i++) {
                try {
                    await collection[i].Init();
                } catch (Exception exception) {
